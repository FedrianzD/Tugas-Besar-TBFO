...
def readHTML(file_path):
    parsing.pathindex = "../test/" + file_path
    parsing.pathlowerindex = f"{parsing.pathindex[0:-5]}lower.html"
def run():
    if len(sys.argv) != 3:
...
while True:
    ...
    if currState == 'qf':
        ...
        break
    if len(input)==0:
       print("Syntax Error, tidak mencapai final state") 
       break
    else:
